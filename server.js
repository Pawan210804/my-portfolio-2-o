...
app.use((req, res, next) => {
  ...
  // Custom Content Security Policy — kept in sync with the <meta http-equiv>
  // CSP in index.html, which is what actually applies on Vercel's static
  // deployment. This header only matters when running server.js locally.
  res.setHeader("Content-Security-Policy", "default-src 'self'; media-src 'self' data: blob: https:; img-src 'self' data: https:; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; script-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net; connect-src 'self' https://api.github.com; worker-src 'self'; frame-ancestors 'none'; upgrade-insecure-requests;");

  next();
});

// Explicit content types for SEO files so they always render/parse correctly
// in the browser instead of falling back to a generic octet-stream type.
