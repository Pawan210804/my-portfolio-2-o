...
app.use((req, res, next) => {
  ...
  res.setHeader("Content-Security-Policy", "default-src 'self'; media-src 'self' data: blob: https:; img-src 'self' data: https:; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; script-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net; connect-src 'self' https://api.github.com; manifest-src 'self'; worker-src 'self'; frame-ancestors *; upgrade-insecure-requests;");

  next();
});

// Service worker must never be cached by an intermediary/browser HTTP cache,
// so updates are detected on the very next visit (the browser still does its
// own byte-for-byte SW update check regardless, this just avoids a stale
// cached copy from a CDN/proxy adding extra delay).
